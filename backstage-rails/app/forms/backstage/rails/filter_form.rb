...
class Backstage::Rails::FilterForm
  ...
  def initialize(attributes = {})
    ...
    self.filter_items << Backstage::Rails::FilterFormItem.new if filter_items.empty?
  end

  def filter_items_attributes=(attributes)
    self.filter_items = attributes.values.map { |attrs| Backstage::Rails::FilterFormItem.new(attrs) }
  end

  def add_filter_item
    filter_items << Backstage::Rails::FilterFormItem.new
  end
...
class Backstage::Rails::FilterFormItem
  include ActiveModel::Model
  include ActiveModel::Attributes
